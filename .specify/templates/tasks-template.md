...
feature touches constitution-protected calculation or validation seams such as
worked hours, payroll, workflow transition validation, inventory calculations,
or financial aggregation. For those seams, include regression tasks where the
repository has an appropriate testing path, plus manual operational checks when
needed.
...
- [ ] T010 Identify authoritative source records, stable identifiers, derived values, and historical records affected by this feature
- [ ] T011 Inspect current Supabase clients, callers, auth behavior, environment variables, and RLS/trusted server boundaries when Supabase access changes
- [ ] T012 Name the brownfield responsibility seam and document current callers, dependent views, calculations, side effects, and validation
...
- [ ] T015 [US1] Regression test for constitution-protected calculation or workflow rule in tests/unit/test_[name].py
...
- [ ] TXXX Verify operational validation checklist from the constitution for affected attendance, payroll, workflow, or financial behavior
- [ ] TXXX Inspect actual package scripts and run relevant available validation commands
...
- Source-record and stable-identifier tasks before derived calculations
- Authorization-boundary tasks before privileged mutations
