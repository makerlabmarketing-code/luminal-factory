...
export function StaffAttendanceContent({ token: propsToken, workerData, assignedBranchData }: any) {
  ...
  const handleToggleShift = () => {
    ...
    if (!assignedBranch) return showToast('Thiếu địa điểm', 'Tài khoản chưa được cấu hình cơ sở làm việc!', 'error');
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      if (distance > assignedBranch.radius) {
        return showToast('Từ Chối Chấm Công', `Bạn đang cách xa cơ sở được giao [${assignedBranch.name}] ${Math.round(distance)}m.`, 'error');
      }
    ...
    }, () => { showToast('Quyền định vị', 'Vui lòng mở quyền vị trí GPS!', 'error'); });
  };

  if (fetching || !worker) return <div className="text-center p-6 text-xs text-slate-500 font-mono"><RefreshCcw className="w-4 h-4 animate-spin text-blue-500 mx-auto mb-2"/> Đang đồng bộ ca trực...</div>;
    ...
    <div className="flex flex-col items-center justify-center p-10 bg-slate-900 border border-slate-800 rounded-3xl space-y-5 shadow-xl max-w-md mx-auto mt-6 animate-fadeIn w-full">
      ...
      <div className="w-full text-left space-y-1">
        ...
        <div className="w-full bg-slate-950 border border-slate-850 p-3 rounded-xl font-sans text-xs text-slate-200 font-black tracking-wide border-l-4 border-l-purple-500 shadow-inner">
          🏛️ {assignedBranch ? assignedBranch.name : 'Chưa được cấu hình cơ sở...'}
        </div>
      </div>
