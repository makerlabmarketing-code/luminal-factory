...
| **17.5. Batch 3C4F: RLS Workflow Slice** | 🔄 | Đang làm audit/pre-run review cho `public.projects`, `public.phases`, `public.tasks`. Chưa chạy SQL, chưa sửa schema/RLS/role/status/dữ liệu. | `Audit schema, relationship, membership model, status vocabulary, UI/server boundary và policy matrix trước khi tạo migration.` | Chờ duyệt migration plan riêng. |
...
| **17.8. Runtime Permission Bridge** | ✅ | Hoàn thành có điều kiện. Runtime server helper đã đọc `employee_workspace_access` và `employee_permissions`, áp dụng DENY thắng ALLOW, admin gate ưu tiên `ADMIN_WORKSPACE` và giữ legacy ADMIN/OWNER fallback. Staff gate dùng `STAFF_WORKSPACE`; `/staff` là Staff Home thật và `/staff/portal` redirect về `/staff`. Navigation hiển thị chuyển khu vực khi user có cả hai workspace. Chưa thay `public.is_app_admin()`, finance RLS, routes mutation, invite/account management hoặc `project_members`. | `Runtime Permission Bridge đã Hoàn thành có điều kiện. Legacy admin fallback vẫn bật.` | Employee List & Mutation Permissions. |
| **17.9. Employee List Read Bridge** | ⚠️ | DEFERRED. Audit xác nhận `app/admin/employees/page.tsx` đang trộn browser read với thêm/sửa/xóa trực tiếp `employees`; chưa tách sạch server DTO trong Slice 3 để tránh mở mutation permission nửa vời. | `Tách employee list read DTO và mutation permissions trong slice riêng.` | Employee List & Mutation Permissions. |
...
- Batch 3C4F RLS Workflow slice: Đang làm audit/pre-run review cho `public.projects`, `public.phases`, `public.tasks`. Chưa chạy SQL, chưa sửa schema/RLS/role/status/dữ liệu. Bước tiếp theo: policy matrix và migration plan cho workflow least-privilege.
...
- Runtime Permission Bridge: Hoàn thành có điều kiện ngày 2026-07-14. App runtime đã có server helper `hasWorkspaceAccess`, `requireWorkspaceAccess`, `hasPermission`, `requirePermission`, `canAccessAdmin`, `canAccessStaff`; admin gate dùng `ADMIN_WORKSPACE` với legacy ADMIN/OWNER fallback; staff gate dùng `STAFF_WORKSPACE`; `/staff` là Staff Home và `/staff/portal` redirect server-side về `/staff`; user có cả `ADMIN_WORKSPACE` và `STAFF_WORKSPACE` thấy link chuyển khu vực. Chưa thay `public.is_app_admin()`, finance RLS, mutation APIs, invite/account management, `project_members` hoặc dữ liệu live.
- Employee List Read Bridge: DEFERRED sau audit Slice 3. Trang `app/admin/employees/page.tsx` hiện trộn browser read với thêm/sửa/xóa trực tiếp `employees`; cần tách server DTO `EMPLOYEE_VIEW` và mutation permissions `EMPLOYEE_MANAGE`/`ACCOUNT_MANAGE` trong slice kế tiếp để tránh chuyển nửa vời.
